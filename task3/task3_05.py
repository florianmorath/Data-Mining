...
def mapper(key, value):
    ...
    Bi = [[] for i in range(k2)] # Bi[j] set of indexes of points from X closest to the jth sampled point
    for i in range(N):
        Bi[Bi_indexes[i]].append(i)
    # Compute the sensitivity for every point in X
    c_phi = min_distances.sum()/N
    min_distances_sum_Bi = [sum([min_distances[x] for x in Bi[i]]) for i in range(k2)]
    Bi_part = [2.0*alpha*min_distances_sum_Bi[i]/(len(Bi[i])*c_phi) + 4.0*N/len(Bi[i]) if Bi[i] else 0 for i in range(k2)]
    ...
    for e in range(epochs):
        ...
        for x,w in coreset:
            ...
            for r in range(restarts):
                point_sum[r][c[r]] += 1*x # update the sum
                point_count[r][c[r]] += 1 # update the count
