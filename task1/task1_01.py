...
# size of one signature = number of hash functions used for min-hashing
min_hash_size = r*b
...
band_hash_bucket_size = 10000000
...
def mapper(key, value):
    ...
    # extract shingles from this page into a list
    ...
    # hash the bands of the signature
    band_hashes = []
    for band in bands:
        temp = []
        for j in range(r):
...
def reducer(key, values):
    ...
    # extract id and shingles from each page
    page_ids = []
    page_shingles = []
    for value in values:
        # extract the page id
    ...
    for i in range(len(values)):
        for j in range(i + 1, len(values)):
            # for each candidadte pair of pages calculate the similarity
